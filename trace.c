...
// Generate a random direction in hemisphere around normal n
vector randomHemisphereDirection(vector n) {
  // Generate random point on unit sphere
  float_t u = ((float_t)rand() / (float_t)RAND_MAX);
  float_t v = ((float_t)rand() / (float_t)RAND_MAX);
  float_t theta = u * 2.0 * M_PI;
  float_t phi = acos(2.0 * v - 1.0);
  vector dir = V3(
      sin(phi) * cos(theta),
      sin(phi) * sin(theta),
      cos(phi)
  );
  // Ensure the direction is in the same hemisphere as normal n
  if (dot(dir, n) < 0) {
      dir = -dir;
  }
  return dir;
...
vector _traceRay(render_context* c, ray_color_pair p, int max_bounces, int max_samples, trace_stats* out_stats) {
    trace_stats tc = {0, 0};
    ...
    for (int n = 0; n < max_bounces; n++) {
        ...
        if (isHit(closest)) {
            ...
            if (!IS_VZERO(closest.m->c_diffuse)) {
              if (max_samples) {
                for (i = 0; i < max_samples; i++, count++) {
                  diffuse_color += _traceRay(c,
                      (ray_color_pair){
                          .r = (ray){closest.p,
                                      randomHemisphereDirection(closest.n)},
                          .c = {0},
                          .m = null_material
                      },
                      max_bounces - n - 1,
                ...
                diffuse_color = diffuse_color * (1.0 / max_samples);
                // Multiply by the material diffuse color
                diffuse_color = diffuse_color * closest.m->c_diffuse;
              } else {
                  for (i = 0; i < c->num_spheres; i++, count++) {
            ...
            //This is our next reflected ray
    ...
    while (block != NO_MORE_BLOCKS) {
        ...
        for (int y = start_y; y < end_y; y++) {
            for (int x = start_x; x < end_x; x++) {
                trace_stats tc;
                ray_color_pair p = _generateOriginRay(c->rc, x, y);
                vector cor = _traceRay(c->rc, p, c->rc->max_bounces, c->rc->diffuse_samples, &tc);
                //Output the pixel into the buffer
                long pixel = x + y * c->rc->x;
                c->rc->output[pixel] = toneMapFloatToColor(cor);
...
int renderScene(render_context* rc, render_parameters* params) {
    ...
    rc->diffuse_samples = params->diffuse_samples;
    rc->cur_itr = 0;
    int x = params->x;
    int y = params->y;
